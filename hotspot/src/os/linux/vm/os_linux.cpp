...
//  first_hrtime = gethrtime();
  ...
  if (os::supports_monotonic_clock()) {
#ifndef __ANDROID__
    // In Android pthread_condattr_setclock is not available:
    // generic pthread_cond_timedwait will be redirected to
...
static void unpackTime(timespec* absTime, bool isAbsolute, jlong time) {
  ...
  if (!os::supports_monotonic_clock() || isAbsolute) {
    ...
    if (isAbsolute) {
#ifdef __ANDROID__
      // Android does not have the pthread_condattr_setclock call.
      // All condition variables use one or the other of monotonic or
      // real time clock.
      // If we have monotonic clock then we need to convert the absolute
      // CLOCK_REALTIME deadline into CLOCK_MONOTONIC form.
      if (os::supports_monotonic_clock()) {
        // Need to convert to absolute in monotonic clock space
        // Get difference from now to time
        ...
        secs = secs - now.tv_sec;
        if (secs < 0) {
          // Negative interval, just set to 0
          secs = 0;
          usecs = 0;
        } else {
          usecs = ((time % 1000) - (now.tv_usec / 1000)) * 1000;
          if (usecs < 0) {
            secs--;
            if (secs < 0) {
              secs = 0;
        ...
        // Now we have the diff from realtime in secs and msecs
        // Get monotonic time now and calculate new absolute timeout value.
        int status = os::Linux::clock_gettime(CLOCK_MONOTONIC, absTime);
        assert_status(status == 0, status, "clock_gettime");
        absTime->tv_sec += secs;
