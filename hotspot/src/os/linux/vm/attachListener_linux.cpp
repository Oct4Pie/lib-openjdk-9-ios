...
#else
  {
    // creating socket in abstract namespace.
    const char *name = "com.oracle.java.se.embedded.attach";
    ...
    sock_len = offsetof(struct sockaddr_un, sun_path) + strlen(name) + 1;
  }
#endif
  res = ::bind(listener, (struct sockaddr*)&addr, sock_len);
  if (res == -1) {
    ::close(listener);
    return -1;
  }
