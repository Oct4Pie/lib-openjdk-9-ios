...
#else
      os_signal = (signal_t)dlsym(RTLD_NEXT, "bsd_signal");
#endif
    } else {
...
#else
  // If you'll look at signal.h under arch-arm/usr/include you'll see that
  // signal() is an inline function that calls to bsd_signal().
  // Therefore, calls to signal are replaced by calls
  // to bsd_signal(). By using bsd_signal() here (instead of signal())
  // I'm achieving the same original intention.

sa_handler_t bsd_signal(int sig, sa_handler_t disp) {
#endif
