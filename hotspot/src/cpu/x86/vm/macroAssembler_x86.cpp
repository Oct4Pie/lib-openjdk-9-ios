...
void MacroAssembler::fast_lock(Register objReg, Register boxReg, Register tmpReg,
  ...
  if (EmitSync & 1) {
    ...
    if (EmitSync & 8192) {
       ...
       movptr(tmpReg, (int32_t)NULL_WORD);        // consider: xor vs mov
       if (os::is_MP()) {
         lock();
       }
    ...
    if ((EmitSync & 128) == 0) {                      // avoid ST-before-CAS
       ...
       if ((EmitSync & 64) == 0) {
         ...
         movptr(tmpReg, (int32_t)NULL_WORD);
       } else {
...
void MacroAssembler::get_vm_result_2(Register metadata_result, Register java_thread) {
  ...
  movptr(Address(java_thread, JavaThread::vm_result_2_offset()), (int32_t)NULL_WORD);
}
