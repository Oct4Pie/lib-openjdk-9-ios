...
public:
  ...
  void movptr(Register dst, int32_t src);
  void movptr(Register dst, Register src);
  void movptr(Address dst, int32_t src);
...
#ifdef _LP64
  // On LP64 the int32_t versions above can be used for moving NULL.
  // Although there are situations in initializing the mark word where
  // they could be used. They are dangerous.

  // On LP64, we need both an int32_t version and an intptr_t (for
  // real pointers). They only exist on LP64, so that int32_t
  // and intptr_t are not the same and we have ambiguous declarations.

  void movptr(Address dst, intptr_t imm32);
  void movptr(Register dst, intptr_t imm32);
#endif // _LP64
