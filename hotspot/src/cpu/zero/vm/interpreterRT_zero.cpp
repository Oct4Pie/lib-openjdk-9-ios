...
// On some of the platforms supported by libffi, arguments smaller than
// 32 bits are not be passed like integers.

void InterpreterRuntime::SignatureHandlerGeneratorBase::pass_bool() {
  push(T_BOOLEAN);
  _cif->nargs++;
}

void InterpreterRuntime::SignatureHandlerGeneratorBase::pass_char() {
  push(T_CHAR);
  _cif->nargs++;
}
